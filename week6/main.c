...
void* simple_malloc(size_t size) {
    ...
    if (remaining_size >= sizeof(simple_metadata_t) + 8) {
...
//   *  |dummy| is a dummy free slot (only used to make the free list
//      implementation myr).
//   *  |next| points to another free slot
...
void* my_malloc(size_t size) {
    ...
    // If there are no available free slots.
    if (!heap) {
        size_t buffer_size = 4096;
    ...
    metadata->next = NULL;
    // The smallest size is 8, so we need at least sizeof(my_metadata_t) + 8.
    if (remaining_size >= sizeof(my_metadata_t) + 8) {
        // Create a new metadata for the remaining free slot.
        //
        // ... | metadata | object | metadata | free slot | ...
        //     ^          ^        ^
